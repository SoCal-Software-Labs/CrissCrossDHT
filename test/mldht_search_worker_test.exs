defmodule CrissCrossDHT.Search.Worker.Test do
  use ExUnit.Case
  ...
  setup do
    ...
    [
      pid: CrissCrossDHT.Registry.get_pid(node_id_enc, CrissCrossDHT.Search.Supervisor),
      node_id: node_id,
  ...
  test "get_peers", state do
    search_pid =
      state.pid
      |> CrissCrossDHT.Search.Supervisor.start_child(:get_peers, nil, state.node_id)
  ...
  test "find_node", state do
    search_pid =
      state.pid
      |> CrissCrossDHT.Search.Supervisor.start_child(:find_node, nil, state.node_id)
  ...
  test "if search exists normally and does not restart", state do
    ...
    Search.stop(search_pid)
    assert CrissCrossDHT.Registry.get_pid(state.node_id_enc, Search, tid_enc) == nil
  end
end
